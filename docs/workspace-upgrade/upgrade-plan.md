...
| 6.2 | Schedule the deterministic maintenance tier | `crontab -l` shows 0 entries; the scripts run only if invoked by hand. | DONE (artifacts) — `crontab.example` already existed; added rebuild-wiki-index + storage-audit jobs, and built `scripts/install-crontab.sh` (idempotent, marked block, --dry-run/--uninstall). NOT auto-installed: installing cron modifies the user env, so it needs an explicit `bash scripts/install-crontab.sh`. |
